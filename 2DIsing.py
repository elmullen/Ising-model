...
def make_lattice(rows,columns): 
    ...
    for i in range(rows):
        for j in range(columns):
            lattice[i][j] = 1 # lattice is filled with +1 spins initially. 
    return lattice

def H_energy(lattice,lattice_size_x, lattice_size_y, x, y):
#Calculates energy of a lattice site, usage of modulo operator instead of cascading if statements
#saves computation time.
    right = lattice[(x+1) % lattice_size_x][y]
    left = lattice[(x-1) % lattice_size_x][y]
    up = lattice[x][(y+1) % lattice_size_y]
    down = lattice[x][(y-1) % lattice_size_y]
#here the hamiltonian is computed as: 
    return -1 * lattice[x][y] * (left + right + up + down)
 
def spin_energy(lattice, lattice_size_x, lattice_size_y, i, j,T): 
    energy = H_energy(lattice, lattice_size_x, lattice_size_y, i, j) # get energy of current site
...
def Metropolis_MonteCarlo():
    ...
    lattice_size_y= 5
    #lattice generation
    lattice = make_lattice(lattice_size_x, lattice_size_y)
    #loop variables    
...
# number of points selected for equiliberation- greater than number of lattice points avalible- ensure every lattice point multiple times- randomisation may not include all points otherwise      
        while i< (lattice_size_x)*(lattice_size_y) * 50: 
# random sampling according to monte carlo method to equiliberate the lattice 
            ...
            lattice = spin_energy(lattice, lattice_size_x,lattice_size_y, x, y, T)[0]
            i += 1
# equlibrium achieved 
        i = 0
        energy = 0
        ...
        while i < (lattice_size_x*lattice_size_y) * samples:  
            x = random.randint(0,lattice_size_x-1)
            y = random.randint(0,lattice_size_y-1)
#collect data in arrays 
            energy_holder = H_energy(lattice,lattice_size_x,lattice_size_y, x, y) # energy value for a lattice site 

            magnetism_array.append(lattice[x][y])# magnetic value for a lattice site 
            ...
            magnetism += lattice[x][y]# add magnetism to total magnetism for averaging 
            i += 1
        energy = energy / ((lattice_size_x * lattice_size_y) * samples) # average of energy over the crystal
        magnetism = magnetism / (((lattice_size_x*lattice_size_y) * samples)) # average of magnetisation over the crystal
...
#converge the data
        ...
        plt.title("Average Lattice Magnetism vs Temperature("+ str(lattice_size_x) + "x" + str(lattice_size_y)  +" lattice)")
        plt.plot(MPI_Tlist, MPI_Mag, 'b--')

        plt.figure(2)
        plt.xlabel("Temperature(J/KbT)")
        plt.ylabel("Average lattice Energy(J)")
        plt.title("Average energy vs Temperature(" + str(lattice_size_x) + "x" + str(lattice_size_y) + " lattice)")
        plt.plot(MPI_Tlist, MPI_Energy, 'b--')
        
        plt.figure(3)
        plt.plot(MPI_Tlist, MPI_magnetic_susceptibility, 'b--')         
        plt.xlabel("Temperature(J/KbT)")
        plt.title("Magnetic Susceptability vs Temperature(J)("+ str(lattice_size_x) + "x" + str(lattice_size_y) + " lattice)")
        ...
        plt.title("Average lattice Energy vs Magnetic Susceptability)("+ str(lattice_size_x) + "x" + str(lattice_size_y) + " lattice)")
        plt.ylabel("Magnetic Susceptability (mu/K)")
 	
        plt.figure(5)
        plt.plot(MPI_Tlist,MPI_specific_Heat_capacity, 'b--')
        plt.xlabel("Temperature(J/KbT)")
        plt.title("Specific Heat vs Temperature("+ str(lattice_size_x) + "x" + str(lattice_size_y) +  " lattice)")
