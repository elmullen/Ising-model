...
# generate lattice 
def make_lattice(rows,columns, width): 
    lattice = np.zeros((rows, columns, width))
    for i in range(rows):
        for j in range(columns):
            for k in range(width):
                lattice[i][j][k] = 1 # lattice is filled with +1 spins initially. 
    return lattice

def H_energy(lattice,lattice_size_x, lattice_size_y, lattice_size_z, x, y, z):
#Calculates energy of a lattice site, usage of modulo operator instead of cascading if statements
#saves computation time.
    right = lattice[(x+1) % lattice_size_x][y][z] 
    left = lattice[(x-1) % lattice_size_x][y][z]
    up = lattice[x][(y+1) % lattice_size_y][z]
    down = lattice[x][(y-1) % lattice_size_y][z]
    forward = lattice[x][y][(z+1) % lattice_size_z]
    back = lattice[x][y][(z-1) % lattice_size_z]
#here the hamiltonian is computed as: 
    return -1 * lattice[x][y][z] * (left + right + up + down + forward + back)
 
def spin_energy(lattice, lattice_size_x, lattice_size_y, lattice_size_z, i, j, k, T): 
    energy = H_energy(lattice, lattice_size_x, lattice_size_y, lattice_size_z, i, j, k) # get energy of current site
...
def Metropolis_MonteCarlo():
    ...
    lattice_size_z = 10 
    #lattice generation
    lattice = make_lattice(lattice_size_x, lattice_size_y, lattice_size_z)
    #loop variables    
...
# number of points selected for equiliberation- greater than number of lattice points avalible- ensure every lattice point multiple times- randomisation may not include all points otherwise      
        while i< (lattice_size_x)*(lattice_size_y)*(lattice_size_z) * 50: 
# random sampling according to monte carlo method to equiliberate the lattice 
            x = random.randint(0,lattice_size_x-1) 
            y = random.randint(0,lattice_size_y-1)
            z = random.randint(0,lattice_size_z-1)
            lattice = spin_energy(lattice, lattice_size_x,lattice_size_y,lattice_size_z, x, y, z, T)[0]
            i += 1
# equlibrium achieved 
        i = 0
        energy = 0
        ...
        while i < (lattice_size_x*lattice_size_y*lattice_size_z) * samples:  
            x = random.randint(0,lattice_size_x-1)
            y = random.randint(0,lattice_size_y-1)
            z = random.randint(0,lattice_size_z-1)
#collect data in arrays 
            energy_holder = H_energy(lattice,lattice_size_x,lattice_size_y,lattice_size_z, x, y, z) # energy value for a lattice site 

            magnetism_array.append(lattice[x][y][z])# magnetic value for a lattice site 
            ...
            magnetism += lattice[x][y][z]# add magnetism to total magnetism for averaging 
            i += 1
        energy = energy / ((lattice_size_x * lattice_size_y*lattice_size_z) * samples) # average of energy over the crystal
        magnetism = magnetism / (((lattice_size_x*lattice_size_y*lattice_size_z) * samples)) # average of magnetisation over the crystal
...
#converge the data
        ...
        plt.title("Average Lattice Magnetism vs Temperature("+ str(lattice_size_x) + "x" + str(lattice_size_y) + "x" + str(lattice_size_z)  +" lattice)")
        plt.plot(MPI_Tlist, MPI_Mag, 'b--')

        plt.figure(2)
        plt.xlabel("Temperature(J/KbT)")
        plt.ylabel("Average lattice Energy(J)")
        plt.title("Average energy vs Temperature(" + str(lattice_size_x) + "x" + str(lattice_size_y) + "x" + str(lattice_size_z) + " lattice)")
        plt.plot(MPI_Tlist, MPI_Energy, 'b--')
        
        plt.figure(3)
        plt.plot(MPI_Tlist, MPI_magnetic_susceptibility, 'b--')         
        plt.xlabel("Temperature(J/KbT)")
        plt.title("Magnetic Susceptability vs Temperature(J)("+ str(lattice_size_x) + "x" + str(lattice_size_y) + "x" + str(lattice_size_z)  + " lattice)")
        ...
        plt.title("Average lattice Energy vs Magnetic Susceptability)("+ str(lattice_size_x) + "x" + str(lattice_size_y) + "x" + str(lattice_size_z) + " lattice)")
        plt.ylabel("Magnetic Susceptability (mu/K)")
 	
        plt.figure(5)
        plt.plot(MPI_Tlist,MPI_specific_Heat_capacity, 'b--')
        plt.xlabel("Temperature(J/KbT)")
        plt.title("Specific Heat vs Temperature("+ str(lattice_size_x) + "x" + str(lattice_size_y) + "x" + str(lattice_size_z) + " lattice)")
